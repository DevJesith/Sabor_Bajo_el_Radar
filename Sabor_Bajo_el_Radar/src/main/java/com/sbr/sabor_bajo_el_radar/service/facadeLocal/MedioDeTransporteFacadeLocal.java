package com.sbr.sabor_bajo_el_radar.service.facadeLocal;

import com.sbr.sabor_bajo_el_radar.model.MedioDeTransporte;

import java.util.List;
import java.util.Optional;

public interface MedioDeTransporteFacadeLocal {

    MedioDeTransporte create(MedioDeTransporte medioDeTransporte);

    MedioDeTransporte update(MedioDeTransporte medioDeTransporte);

    void delete(Integer id);

    Optional<MedioDeTransporte> find(Integer id);

    List<MedioDeTransporte> findAll();
}
